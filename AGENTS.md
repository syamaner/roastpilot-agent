...
- **Skills** (`.claude/skills/` and shared `.agents/skills/`) — read the relevant
  ...
  | `capture-agent-usage` | Opt-in, top-level-Codex-parent-only metadata capture. `run` remains measurement/validation-only; D163's separate `run-native-claude` binds the version probed from the installed authenticated CLI alongside committed effort for the eligible role roster, derives its read/write capability from committed tools, reads only its bound parent session after exit, rejects any subagent tree, and writes verified metadata without becoming routing authority. `ui-reviewer` and `repair` are excluded. D166 adds a bounded, transcript-validated READ_ONLY handback returned to the launching parent only (never persisted, and treated as untrusted inert data), and a parent-provisioned external `--validation-root` for the three test-running READ_ONLY roles (`qa`, `mcp-contract-checker`, `sim-roast-runner`) whose attested worktree must stay byte-clean. D167 derives, from that same one validated root, exactly one `--add-dir <validated real root>` argv pair for those three roles' native launch, placed immediately before `--permission-mode`; it grants path access, not tools, and every other native role's argv is unchanged. D168 adds one committed, role-fixed `--allowedTools` allow-list for the same three roles — rendered only from that same validated root through one committed table, appended last in the argv — because `--add-dir` alone left every gate command denied by the provider's `dontAsk` default; the parent renders each role's exact per-run commands with `print-validation-commands` from that same table, so the argv rules and the lead-authored brief can never diverge, and committed frontmatter `tools:` remains the sole capability boundary. D169 generalizes the bound-root abstraction to two more closed role sets — `--plan-root`/`--plan-sha` (required for `planning-architect`, `story-planner`, and `product-auditor`) and `--evidence-root`/`--evidence-pr` (required for `pr-triage`, replacing any live `gh`/network path for that role with a parent-built, manifest-hashed nine-file PR evidence bundle) — with at most one bound root active per launch (the three kinds' admitted role sets are pairwise disjoint), neither PLAN nor EVIDENCE ever rendering `--allowedTools`, and `print-validation-commands` now emitting `ALLOW EXACT`/`ALLOW PREFIX` descriptor lines followed by one concrete `RUN <command>` line per gate (plus an optional repeatable `--pytest-arg` shell-quoted into the `qa` `pytest` gate's `RUN` line) so a validation role can distinguish authorization text from what it actually executes. |
