...
_C7_DTR_PACE_SECTION = (
    ...
    "and the indicated bitter/emergency ceiling as the two numbers from "
    "context and COMPUTE the gap between them (ceiling minus bean "
    "temperature) - act on that computed gap, never on a felt sense that the "
    "ceiling is 'close' or 'approaching'. Waiting until that gap reaches "
    "zero is TOO LATE: a drop takes real seconds to act, and the bean keeps "
    "climbing at its current rate of rise for that whole stretch, so a drop "
    "commanded exactly at the ceiling lands past it. Compare the computed "
    "gap against how far the bean's CURRENT rate of rise (bean_ror_c_per_min "
    "from context) would carry it in the time a drop takes to act - the "
    "ceiling forces the drop the moment that comparison shows the gap is no "
    "longer larger than that projected climb, exactly as the joint-objective "
    "section teaches, regardless of where DTR or temperature stand - but "
    "only that computed comparison decides it, never a felt sense of "
...
_C8_PACE_BOTTOM_EDGE_AND_FAN_SECTION = (
    ...
    "the indicated bitter/emergency ceiling as the two numbers from context "
    "and COMPUTE the gap between them (ceiling minus bean temperature) - act "
    "on that computed gap, never on a felt sense that the ceiling is 'close' "
    "or 'approaching'. Waiting until that gap reaches zero is TOO LATE: a "
    "drop takes real seconds to act, and the bean keeps climbing at its "
    "current rate of rise for that whole stretch, so a drop commanded "
    "exactly at the ceiling lands past it. Compare the computed gap against "
    "how far the bean's CURRENT rate of rise (bean_ror_c_per_min from "
    "context) would carry it in the time a drop takes to act - the ceiling "
    "forces the drop the moment that comparison shows the gap is no longer "
    "larger than that projected climb, regardless of where DTR or "
    "temperature stand - but only that computed comparison decides it, "
    "never a felt sense of nearness and never waiting for the gap to "
    "literally hit zero.\n"
    "\n"
