...
_CONTROL_TEACHING_PROMPTS: dict[str, str] = {
    "c1": (
        ...
        "confidence between 0 and 1, and a short rationale). A deterministic "
        "controller decides whether to apply your advice, and a safety policy "
        "clamps or rejects it first. Never assume your numbers reach the "
        "roaster unchanged.\n"
        "\n"
        "THE MACHINE\n"
...
_C7_DTR_PACE_SECTION = (
    ...
    "Before you weigh the ceiling at all, STATE the current bean temperature "
    "and bitter_ceiling_temp_c from context (never emergency_drop_temp_c - "
    "that is the separate emergency/e-stop bound, not the planning ceiling "
    "for this comparison) and COMPUTE the gap between them "
    "(bitter_ceiling_temp_c minus bean temperature) - act on that computed "
    "gap, never on a felt sense that the ceiling is 'close' or "
    "'approaching'. Frame the gap in the SAME per-minute units as the "
    "climb: bean_ror_c_per_min from context already IS the bean's current "
    "climb per minute. When the remaining gap to bitter_ceiling_temp_c is "
    "only a small fraction of that per-minute climb, the drop must be "
    "forced NOW, not once the gap reaches zero - the command acts with "
    "delay, and waiting for the gap to close first guarantees crossing the "
    "ceiling before the drop takes effect. Only that computed comparison "
    "decides it, exactly as the joint-objective section teaches, "
    "regardless of where DTR or temperature stand - never a felt sense of "
    "nearness and never waiting for the gap to literally hit zero.\n"
...
_C8_PACE_BOTTOM_EDGE_AND_FAN_SECTION = (
    ...
    "teaches for a temperature-ahead / development-behind mismatch. Before "
    "you weigh the ceiling at all, STATE the current bean temperature and "
    "bitter_ceiling_temp_c from context (never emergency_drop_temp_c - that "
    "is the separate emergency/e-stop bound, not the planning ceiling for "
    "this comparison) and COMPUTE the gap between them (bitter_ceiling_"
    "temp_c minus bean temperature) - act on that computed gap, never on a "
    "felt sense that the ceiling is 'close' or 'approaching'. Frame the gap "
    "in the SAME per-minute units as the climb: bean_ror_c_per_min from "
    "context already IS the bean's current climb per minute. When the "
    "remaining gap to bitter_ceiling_temp_c is only a small fraction of "
    "that per-minute climb, the drop must be forced NOW, not once the gap "
    "reaches zero - the command acts with delay, and waiting for the gap "
    "to close first guarantees crossing the ceiling before the drop takes "
    "effect. Only that computed comparison decides it, regardless of where "
    "DTR or temperature stand - never a felt sense of nearness and never "
    "waiting for the gap to literally hit zero.\n"
    "\n"
...
class PydanticAIAdvisor(RoastAdvisor):
    ...
    def __init__(self, config: AdvisorConfig, *, model: Model | None = None) -> None:
        ...
        #: ``Model`` varies. With the empty override map (D130) every phase
    ...
    def descriptor(self) -> AdvisorDescriptor:
        ...
        override map, D130, every phase resolves to this slug, and it remains
    ...
    async def get_recommendation(self, context: AdvisorContext) -> RoastDecision:
        ...
        (D130) this is the configured ``model_slug`` in every phase. The
