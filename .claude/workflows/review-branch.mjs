...
Finally, set \`addsProviderCallPath\` for the narrower CONTENTION case, which routes the safety lens. The test is capability to contend with the roast advisor, NOT whether the remote backend is byte-identical: a path to a *separate* model service still contends if it can consume the same host CPU, event loop, memory, network, or provider rate limit during an active roast (checklist class 6 explicitly covers bounding provider AND CPU contention). Set it true if the new path could run while a roast is in progress and compete for any of those. "New path" means new REACHABILITY, not a new call site: a diff adding an endpoint, route, job, or service method that reaches a provider through an EXISTING helper still counts. Set it false only for a diff that reaches no model provider at all.
