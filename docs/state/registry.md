...
to main at `44996e047f9b76ec702ecf535448da03b738e859`. This closure supersedes
the in-progress or stays-open-pending language in all earlier #702 (D180)
entries below, which are now historical.
...
the exact dev-only `pyright[nodejs]` and `nodejs-wheel-binaries` pins are a
coupled bump that resolves Node through venv-contained `nodejs_wheel`, without
