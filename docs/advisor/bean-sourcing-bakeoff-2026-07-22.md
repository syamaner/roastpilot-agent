...
2.00`; total usage-priced spend $0.5066, breaker never tripped, zero timeouts,
...
- **The blend pages remain the discriminator:** only `gpt-5-mini` and `haiku`
  (and light-arm `luna`) survive both blend fixtures; most arms ERR/MIS them.
...
- **Cost/latency reality check:** actual usage-priced costs ran 1.2–12× the
  chars/4 estimates (worst: `gpt-5-nano` at 12×, provider-default reasoning
  tokens) — the estimate-vs-actual gap the #601 harness now measures and bounds.
