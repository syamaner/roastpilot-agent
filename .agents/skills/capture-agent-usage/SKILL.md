...
`prepare-native-codex` and `finalize-native-codex` are the separate parent-only
metadata path for native registered Codex dispatch. They admit only
`engineer-be`, `engineer-fe`, and `repair`; generic `codex exec` is never a
registered-role launch. Prepare must run before named-role dispatch with the
parent `CODEX_THREAD_ID`; finalize consumes that one pre-binding after exit,
reads only a newly-created provider rollout under the standard provider root,
and records no provider bytes or paths. Both commands require the same owned
external `--usage-root`; `CODEX_HOME` overrides and replayed bindings fail
closed. The parent, not this utility, owns native dispatch.
