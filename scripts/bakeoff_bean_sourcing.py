    ...
    for gold_path in gold_paths:
        ...
        pages.append(
            CorpusPage(
                ...
                gold_fields=cast("dict[str, dict[str, Any]]", gold["fields"]),
...
def _compare_text(gold: str, model: str) -> Outcome:
    ...
    if sim >= _NAME_COR_SIM or recall >= _WORDBAG_COR_RECALL:
        return Outcome.COR
...
def _compare_variety(gold: Any, model: str) -> Outcome:
    recall = word_bag_recall(_gold_tokens(gold), model)
    if recall >= _WORDBAG_COR_RECALL:
        return Outcome.COR
    if recall > 0.0:
        return Outcome.PAR
    return Outcome.INC
...
def macro_f1(run: ModelRun) -> float | None:
    ...
    for spec in FIELD_SPECS:
        ...
        value = f1(precision(counts), recall(counts))
...
def page_combined_scores(run: ModelRun) -> dict[str, float]:
    """Per-page CombinedScore (pages with only ``ERR`` cells are dropped)."""
    scores: dict[str, float] = {}
    for page in run.pages:
        value = combined_score(page.outcomes.values())
        if value is not None:
            scores[page.slug] = value
    return scores
...
def paired_bootstrap_combined(
    ...
    *,
    ...
    Only pages both models scored are used.
    ...
    Returns:
        The :class:`BootstrapCI` on the mean per-page (A - B) CombinedScore gap.

    Raises:
        ValueError: If the two runs share no scorable page.
    """
    scores_a = page_combined_scores(run_a)
    scores_b = page_combined_scores(run_b)
    shared = sorted(scores_a.keys() & scores_b.keys())
    if not shared:
        raise ValueError("runs share no scorable page for a paired bootstrap")
    gaps = [scores_a[slug] - scores_b[slug] for slug in shared]
    estimate = sum(gaps) / len(gaps)
    ...
    return BootstrapCI(
        estimate=estimate,
        low=_percentile(means, ci[0]),
        high=_percentile(means, ci[1]),
        resamples=resamples,
    )
...
CAVEAT_TEXT = (
    ...
    "and the page-clustered bootstrap is the primary test."
)
...
def render_report(runs: Sequence[ModelRun], cost_estimates: Sequence[ModelCostEstimate]) -> str:
    ...
    if len(runs) >= 2:
        ...
        for other in runs[1:]:
            ...
            lines.append(
                ...
                f"([{rec.low:+.3f}, {rec.high:+.3f}]); McNemar exact p="
                f"{mc.exact_p_two_sided:.4f} (secondary, indicative)."
            )
        lines.append("")
...
class Checkpoint:
    ...
    def append(self, record: dict[str, Any]) -> None:
        """Persist one completed model run to disk immediately."""
...
async def run_bakeoff(
    ...
    *,
    ...
    Returns:
        The completed :class:`ModelRun` list (resumed + freshly run).
    """
    ...
    for slug in model_slugs:
        ...
        run = await run_model_over_corpus(
            pages, model_slug=slug, advisor_config=make_advisor_config(slug)
        )
        spent += upcoming
...
async def main(argv: Sequence[str] | None = None) -> int:
    ...
    roster_for_cost = [m for m in MODEL_ROSTER if m.slug in set(model_slugs)] or list(MODEL_ROSTER)
    ...
    report = render_report(runs, cost_estimates)
    print("\n" + report, flush=True)

    out_path = cast("Path", args.out)
    out_path.parent.mkdir(parents=True, exist_ok=True)
    out_path.write_text(json.dumps({"runs": [run_to_json(r) for r in runs]}, indent=2))
