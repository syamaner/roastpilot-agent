// forked from design-sync lib/emit.mjs — exclude --tw-* Tailwind internals and
// scope token extraction to :root/[data-*] (see collectTokenNames). Touches
// ONLY the generated README's "## Tokens" prose, never the @dsCard / bundle
// header / .d.ts output contract the app's self-check reads, so the
// "don't fork emit.mjs" rule is not violated in substance. Relative lib imports
// are repointed at the staged scripts (../../.ds-sync/lib/); the bare `esbuild`
// import resolves via the .design-sync/node_modules fork symlink. On re-sync,
// diff this against the bundled lib/emit.mjs and re-apply the patch if upstream
// moved (this is a full-file fork — the diff is total).
...
// Scope-aware token scrape. A CSS custom property only counts as a design
// token when it is declared on a :root or [data-*] selector (the theme
// scopes). Two classes of declaration are excluded as build artifacts, not
// tokens:
//   - the `--tw-*` namespace — Tailwind's machinery vars (--tw-translate-x,
//     --tw-ring-*, --tw-shadow*, --tw-backdrop-*, …), skipped by name; and
//   - anything declared under a non-token selector — Tailwind re-declares its
//     machinery on `*,::before,::after` and under utility selectors
//     (`::backdrop`, `.focus\:ring-1:focus`, …), and component vars live on
//     component selectors; none of those are token scopes.
// This keeps the registered set to the real tokens (--background, --card,
// --border, --roast-*, --radius, --font-*) and mirrors (pre-empting) the
// claude.ai/design app's downstream scope filter, whose permissive heuristic
// otherwise lets the bundle's machinery vars pollute the token list
// (see css.mjs writeStylesCss). A rule-block qualifies when any selector in
// its comma list mentions `:root` or a `[data-` attribute selector — so
// `:root`, `:root[data-theme="dark"]`, `html[data-mode]` all count, while
// `*,::before,::after` and utility/component selectors do not.
function collectTokenNames(cssText, into) {
  for (const [, selectorList, body] of cssText.matchAll(/([^{}]+)\{([^{}]*)\}/g)) {
    const isTokenScope = selectorList
      .split(',')
      .some((s) => s.includes(':root') || s.includes('[data-'));
    if (!isTokenScope) continue;
    for (const m of body.matchAll(/(?<![\w-])(--[A-Za-z][\w-]*)\s*:/g)) {
...
export function emitReadme({ OUT, GLOBAL, PKG, VERSION, TOKENS_PKG, components, tokenFiles, hasProvider, PROVIDER, hasDecorators = false, jsdocFor, compoundsFor, guidelineCount = 0, headerText = '' }) {
  ...
  for (const f of tokenFiles) {
    collectTokenNames(readText(join(OUT, 'tokens', f)), tokenNames);
  }
  ...
  if (tokenNames.size === 0 && hasBundleCss) {
    collectTokenNames(bundleCssText, tokenNames);
